	...
	<div>
		<div class="flex items-center flex-shrink-0 p-4 pb-0">
			<div class="flex w-12 items-top">
				<img :src="props.user?.profileImage ? props.user?.profileImage : ''" alt=""
					class="inline-block w-10 h-10 rounded-full" />
			</div>

			<div class="w-full p-2">
				<textarea v-model="text" :placeholder="placeholder"
					class="w-full text-lg text-gray-900 placeholder:text-gray-400 bg-transparent border-0 dark:text-white dark:bg-dim-800 focus:ring-0"></textarea>
			</div>
		...
		<!--File Selector-->
		<div class="p-4 pl-16">
			<img :src="inputImageUrl" v-if="inputImageUrl" alt="" class="rounded-2xl border twitterBorderColor	" />
			<input @change="handleImageChange" type="file" ref="imageInput" hidden accept="image/png,image/jpeg,image/gif" />
		</div>
		<!--ICONS-->
		<div class="flex justify-between p-2 pl-14">
			<div class="flex w-full">
				<div @click="handleImageClick"
					class="defaultTransition p-2 cursor-pointer text-blue-400 rounded-full hover:bg-blue-50 dark:hover:bg-dim-800">
			...
			<div>
				<UIButton :disabled="isDisabled" :size="'sm'" @click="handleFormSubmit"><span class="font-bold">Tweet</span>
				</UIButton>
			</div>
		</div>
	</div>
